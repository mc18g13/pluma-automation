...
class Xray:
    ...
    def create_or_update_test_execution(self, test_execution_info, test_exec_key=None, tests=None):
        tests = [] if tests is None else tests
        xray_token = self.xray_athenticate()

        data = {"info": test_execution_info}
        if tests:
            data["tests"] = tests
        if test_exec_key:
            data["testExecutionKey"] = test_exec_key
        resp = requests.post(
            url="https://xray.cloud.xpand-it.com/api/v2/import/execution",
...
class JiraResult:
    ...
    stopDate: datetime
    status: str
    # This must be a non-emptey string or the XRay API will fail to validate the request
    # recommended that it be set to the name.
    summary: str
    comment: str = 'N/A'
    description: str = 'N/A'
    log: str = ''
...
class JiraXrayHelper:
    ...
    def add_attachment(self, issue_key, filename):
        # with _attachement(filename) as (f, _):
    ...
    def create_or_update_test_execution(self, plan, xray_test_exec_info,
        ...
        def update_test(test, key):
            fields = {
                "summary": test.summary,
                "description": test.description
            }
            # logging.debug(f"need to update {test.name}")
            self.jira.update_issue_field(key, fields=fields)
        ...
        unknown_tests = [test for test in xrays_tests if test.key is None]
        if unknown_tests:
            ...
            for t in unknown_tests:
                ...
                if et:
                    ...
                    if t.steps:
                        obj["steps"] = [{"action": step.action} for step in t.steps]
                    bulk_data.append(obj)
            if bulk_data:
                self.xray.import_tests(bulk_data)
                existing_tests = get_existing_automated_test(plan.project_key)
                for t2 in unknown_tests:
                    et2 = find_by_label_in_existing_tests(t2.name, existing_tests)
                    t2.key = et2.key
